...
public class Ihm {
    ...
    public void modifLieu(Connection connection, LieuDAO lieuDAO) {
        try {
            // Affichez la liste des lieux pour que l'utilisateur puisse faire un choix
            afficheList(this.listLieu);
            System.out.println("quelle lieu voulez-vous modifier : ");
            int entry = s.nextInt();

            // Récupérez le lieu sélectionné par l'utilisateur
            Lieu selectedLieu = this.listLieu.get(entry - 1);

            // Affichez les détails du lieu avant la modification
            System.out.println(selectedLieu);

            // Récupérez les nouvelles informations du lieu
            ...
            // Créez un nouveau lieu avec les nouvelles informations
            Lieu updatedLieu = new Lieu(nom, adresse, capacite);

            // Mettez à jour le lieu dans la liste
            this.listLieu.set(entry - 1, updatedLieu);

            // Mettez à jour le lieu dans la base de données
            lieuDAO.updateLieu(selectedLieu.getId(), updatedLieu);

            // Affichez les détails mis à jour du lieu
    ...
    public void suprLieu(Connection connection, LieuDAO lieuDAO) {
        try {
            // Reste du code de votre méthode suprLieu
            int entry = s.nextInt();

            // Reste du code pour la suppression du lieu
