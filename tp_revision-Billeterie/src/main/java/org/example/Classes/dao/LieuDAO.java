...
public class LieuDAO {
        ...
        try (PreparedStatement pst = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet generatedKeys = pst.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    ...
                    // Vous pouvez faire quelque chose avec l'ID généré si nécessaire
                    // Par exemple, this.id = id; si vous souhaitez stocker l'ID généré dans la classe LieuDAO
                } else {
    ...
    public Lieu getLieuByName(String lieuNom) {
        // Implémentez la récupération d'un lieu par son nom si nécessaire
        return null;
    }
}
